...
while ($count <= $num_phones)
  ...
  HInit -m 1 -C Configs/config-init -X lab -S lists/train.mfclist -M hmm -l $phone protos/$2
  sed s/$2/$phone/ hmm/$2 > hmm/$phone
  
  HRest -m 1 -C Configs/config-init -X lab -S lists/train.mfclist -M hmm/reestimatedhmms/ -l $phone hmm/$phone
  @ count++
end
